...
def process_sq_hourly(input_path):
    print(f"🚀 開始處理 Site Quality 檔案: {input_path}")
    ...
    try:
        ...
        if den in hourly_df.columns:
            # % RVI
            if 'Reduced Value Inventory (RVI) Ads' in hourly_df.columns:
                hourly_df['% Reduced Value Inventory (RVI) Ads'] = (hourly_df['Reduced Value Inventory (RVI) Ads'] / hourly_df[den]).fillna(0) * 100
            
            # % seeThrough
            if 'seeThrough Ads' in hourly_df.columns:
                hourly_df['% seeThrough Ads'] = (hourly_df['seeThrough Ads'] / hourly_df[den]).fillna(0) * 100
                
            # Invisible URL rate
            if 'Invisible URL Ads' in hourly_df.columns:
                hourly_df['Invisible URL rate'] = (hourly_df['Invisible URL Ads'] / hourly_df[den]).fillna(0) * 100
                
            # 子項 (Incentivized, Proxy)
            for sub in ['Incentivized Browsing (RVI) Ads', 'Proxy Server (RVI) Ads']:
